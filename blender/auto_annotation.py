...
def ensure_directories():
    ...
    if ENABLE_ANNOTATION:
        ...
        os.makedirs(f"annotation/{SETNAME}/label_cuboid", exist_ok=True)
        os.makedirs(f"annotation/{SETNAME}/label_vertice_obscured", exist_ok=True)
        os.makedirs(f"annotation/{SETNAME}/label_cuboid_obscured", exist_ok=True)
...
def annotate_frame(scene, frame):
    ...
    label_filepath = {
        ...
        'cuboid_bbox': f"annotation/{SETNAME}/label_cuboid/{frame+FRAME_OFFSET}.txt",
        'vertice_bbox_obs': f"annotation/{SETNAME}/label_vertice_obscured/{frame+FRAME_OFFSET}.txt",
        'cuboid_bbox_obs': f"annotation/{SETNAME}/label_cuboid_obscured/{frame+FRAME_OFFSET}.txt"
    }
...
def create_data(camera, scene, obj, label_save):
    '''
    Dataset format:
    <class> <cx> <cy> <w> <h> <px1> <py1> <pv1> <px2> <py2> <pv2> ... <pxn> <pyn> <pvn>

    <class>   Object class index: An integer representing the class of the object (e.g., 0 for person, 1 for car, etc.).
    ...
    <px> <py> Object keypoint coordinates: The keypoints of the object, normalized to be between 0 and 1.
    <pv>      Object keypoint visibility: The visibility of keypoint, either 0 (invisible) or 2 (visible).
    ...
    for corner in bbox_corners:
        pos = bpy_extras.object_utils.world_to_camera_view(scene, camera, corner)

        # Sanity check: point inside camera view and not obscured
        if not (0 < pos.x < 1 and 0 < pos.y < 1):
           projected_points.append([0.0, 0.0, 0.0]) # Outside camera view
        else:
            ray_dir = (corner - camera_location).normalized()

            # Visibility test
            hit, location, _, _, object_hit, _ = scene.ray_cast(bpy.context.view_layer.depsgraph, camera_location, ray_dir)

            if hit and object_hit != obj and (location - camera_location).length < (corner - camera_location).length:
                projected_points.append([pos.x, 1 - pos.y, 1.0]) # Obscured by other object
            else:
                projected_points.append([pos.x, 1 - pos.y, 2.0]) # Clear view

        minX, maxX = min(minX, pos.x), max(maxX, pos.x)
        minY, maxY = min(minY, pos.y), max(maxY, pos.y)

    x_center_c = (minX + maxX) / 2
    y_center_c = 1 - (minY + maxY) / 2  # Invert Y-axis
    width_c, height_c = maxX - minX, maxY - minY

    # Sanity check: Object inside camera view
    if not (0 < x_center_c < 1 and 0 < y_center_c < 1):
        return  # Simply do nothing (there is nothing to add)

    # Add centroid to projected bbox array
    bbox_centroid = sum(bbox_corners, mathutils.Vector()) / len(bbox_corners)
    centroid_pos = bpy_extras.object_utils.world_to_camera_view(scene, camera, bbox_centroid)

    # Sanity check: point inside camera view and not obscured
    if not (0 <= centroid_pos.x <= 1 and 0 <= centroid_pos.y <= 1):
        projected_points.append([0.0, 0.0, 0.0]) # Outside camera view
    else:
        ray_dir = (bbox_centroid - camera_location).normalized()

        # Visibility test
        hit, location, _, _, object_hit, _ = scene.ray_cast(bpy.context.view_layer.depsgraph, camera_location, ray_dir)

        if hit and object_hit != obj and (location - camera_location).length < (bbox_centroid - camera_location).length:
            projected_points.append([pos.x, 1 - pos.y, 1.0]) # Obscured by other object
        else:
            projected_points.append([centroid_pos.x, 1 - centroid_pos.y, 2.0]) # Clear view
    ...
    if class_label != -1:
        with open(label_save['vertice_bbox'], 'a') as f:
            ...
            for px, py, pv in projected_points:
                f.write(f"{px} {py} {pv if pv != 1.0 else 2.0} ")

            f.write("\n")

        with open(label_save['cuboid_bbox'], 'a') as f:
            f.write(f"{class_label} {x_center_c} {y_center_c} {width_c} {height_c} ")
        ...
        with open(label_save['vertice_bbox_obs'], 'a') as f:
            f.write(f"{class_label} {x_center_v} {y_center_v} {width_v} {height_v} ")

            for px, py, pv in projected_points:
                f.write(f"{px} {py} {pv if pv != 1.0 else 0.0} ")

            f.write("\n")

        with open(label_save['cuboid_bbox_obs'], 'a') as f:
            f.write(f"{class_label} {x_center_c} {y_center_c} {width_c} {height_c} ")
...
#     blur_node.size_x, blur_node.size_y = GAUSSIAN_BLUR
...
#for randlight_opt in [False, True]:
#    for bgpatt_opt in ['SOLID', 'GRID', 'NOISE', 'VORONOI']:
#        ENABLE_RANDOM_LIGHTING = randlight_opt
#        BACKGROUND_PATTERN = bgpatt_opt
#
#        SETNAME = compose_setname()
